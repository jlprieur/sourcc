...
*/
    printf("No color compression was necessary.\n");
    return pic8;   
  }
...
*/
  ...
  if (!pixels) 
/* JLP97 FatalError("couldn't allocate 'pixels' array");
*/
             exit(-1); 
  for (row=0; row<rows; row++) {
    ...
    if (!pixels[row]) 
/* JLP97
FatalError("couldn't allocate a row of pixels array");
*/
             exit(-1); 
...
*/
	if (!chl) {printf("ran out of memory adding to hash table");
                   exit(-1);}
     ...
     int maxval;
{
  ...
  if (!bv || !colormap) 
/* JLP97: FatalError("unable to malloc in mediancut()");
*/
         {printf("unable to malloc in mediancut()\n"); exit(-1);}
     ...
     int* colorsP;
{
  ...
  *colorsP = 0;
  ...
  for (row=0; row<rows; row++)
	...
	if (!chl) 
/* JLP97
FatalError("ran out of memory computing hash table");
*/
        {printf("ran out of memory computing hash table");
        exit(-1);}
