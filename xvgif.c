...
static float  normaspect;    /* normal aspect ratio of this picture */
static int    numcols;      /* # of desired colors in picture */
static int DEBUG=1;
/* EOF JLP97 */
...
static int   gifError    (PICINFO *, char *);
static void  gifWarning  (char *);
static void xvbzero (char *, size_t);
...
*/
	 printf("PlainText extension found in GIF file.  Ignored.\n");
...
*/
	 printf("Graphic Control Extension in GIF file.  Ignored.\n");
...
/* JLP97
	SetISTR(ISTR_INFO,
		...
		bname, fn);
*/
          printf("Unknown extension 0x%02x in GIF file.  Ignored.",fn);
...
*/
    printf("No colormap in this GIF file.  Assuming EGA colors.");
  }
...
*/
      printf("This GIF file seems to be truncated.  Winging it.\n");
      break;
    }
  } while(ch1);
...
*/
  ww = pic8;
  for(k = 0; k < npixels; k++)
    {real_array[k] = (float)pinfo->b[*ww++];}
  strcpy(outfile,"aaa");
  strcpy(outcomments,"Converted from GIF");
  JLP_WRITEIMAG(real_array,&Width,&Height,&Width,outfile,outcomments);
/* EOF JLP97 */
...
/* JLP97, Warning, *ptr = pic8[YC*Width] at the beginning of each line ... 
  if (oldYC != YC) {  ptr = pic8 + YC * Width;  oldYC = YC; }
* I prefer:
*/
  ptr = pic8 + YC * Width + XC ;
  
/* JLP97, Warning, *ptr++ = pic8[YC*Width + XC + 1] now ... */
  if (YC<Height) *ptr++ = Index;
