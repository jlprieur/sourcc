...
char colortable[3][256][3];         /* RGB intensity strings for an image */
...
for (i=0; i<ncolors; i++)
  {
  color = colormap[i][0];
  colortable[0][i][0] = hextab[color >> 4];
  colortable[0][i][1] = hextab[color & 15];
  colortable[0][i][2] = '\0';
  color = colormap[i][1];
  colortable[1][i][0] = hextab[color >> 4];
  colortable[1][i][1] = hextab[color & 15];
  colortable[1][i][2] = '\0';
  color = colormap[i][2];
  colortable[2][i][0] = hextab[color >> 4];
  colortable[2][i][1] = hextab[color & 15];
  colortable[2][i][2] = '\0';
  }
}
/***************************************************************************/
  ...
  for (k=0; k<3; k++)
    ...
    for (i=top; i<top+height; i++)
      ...
      for (j=0; j<width; j++)
        ...
        fprintf(fp_out,"%s",colortable[k][val]);
        }
      }
    fprintf(fp_out,"\n>}");
    }
...
if (argc == 1)
  {
  (void) printf("\nUSAGE:\n");
  (void) printf("gif2psc input_file output_file\n\n");
